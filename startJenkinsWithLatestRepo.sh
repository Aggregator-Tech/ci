...
if [ -z "$platformurl" ]; then
   echo "No platform url from cmd"
   platformurl="aggregatortech-platform.herokuapp.com"
  
fi

echo "Proxy host is"+$proxyHost
echo "Proxy Port is"+$proxyPort
echo "Platfrom url is"+$platformurl
...
if [ $proxyHost != "--noProxy" ] ; then
   ...
   sed "s/\${http.proxyHost}/$proxyHost/;s/\${http.proxyPort}/$proxyPort/;s/\${platform_url}/$platformurl/"g aggregatortech-jenkins-dv-repo/jenkins_home/config.xml >>  stagingForJenkins/jenkins_home/config.xml
